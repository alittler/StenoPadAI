...
app.get('/auth', async (req, res) => {
    try {
        const authUrl = await dbx.auth.getAuthenticationUrl({
            redirectUri: 'https://steno-backend.onrender.com/auth/callback',
            tokenAccessType: 'offline',
        });
        res.redirect(authUrl);
    } catch (error) {
        console.error('Error getting auth URL:', error);
        res.status(500).send('Error getting authentication URL.');
    }
});
...
app.get('/auth/callback', async (req, res) => {
    ...
    try {
        const { result } = await dbx.auth.getAccessTokenFromCode(
            'https://steno-backend.onrender.com/auth/callback',
            code
        );
